...
public final class MekanicalFactoryMultiblockData extends MultiblockData {
    ...
    public boolean tick(Level level) {
        ...
        FloatingLong energyPerTick = FloatingLong.create(getEnergyPerTick());
        ...
        if (energyContainer.extract(energyPerTick, Action.SIMULATE,
                AutomationType.INTERNAL).compareTo(energyPerTick) != 0) {
    ...
    private boolean tickOverflowingPlan(Level level, boolean needsPacket) {
        ...
        while (operations < MAX_OVERFLOW_OPERATIONS_PER_TICK) {
            ...
            FloatingLong operationEnergyAmount = FloatingLong.create(operationEnergy);
            if (energyContainer.extract(operationEnergyAmount, Action.SIMULATE,
                    AutomationType.INTERNAL).compareTo(operationEnergyAmount) != 0) {
                starved = true;
            ...
            energyContainer.extract(operationEnergyAmount, Action.EXECUTE,
    ...
    private boolean canFit(List<ItemStack> results, List<FluidStack> fluidResults) {
        ...
        for (FluidStack result : fluidResults) {
            ...
            for (int index = 0; index < fluidSimulation.size() && remaining > 0; index++) {
                ...
                if (!stored.isEmpty() && stored.isFluidEqual(result)
                        && FluidStack.areFluidStackTagsEqual(stored, result)) {
            ...
            for (int index = 0; index < fluidSimulation.size() && remaining > 0; index++) {
                if (fluidSimulation.get(index).isEmpty()) {
                    ...
                    fluidSimulation.set(index, new FluidStack(result, accepted));
                    remaining -= accepted;
                }
            }
            if (remaining > 0) {
    ...
    public long getEnergy() {
        return isRemote() ? clientEnergy : energyContainer.getEnergy().longValue();
    }
    ...
    public int getSpeedCoreCount() {
        return getClientStructureCoreCount(ModBlocks.MEKANICAL_FACTORY_SPEED_CORE.getBlock(),
                speedCoreCount, Integer.MAX_VALUE);
    }

    public int getEnergyCoreCount() {
        return getClientStructureCoreCount(ModBlocks.MEKANICAL_FACTORY_ENERGY_CORE.getBlock(),
                energyCoreCount, Integer.MAX_VALUE);
    }

    public int getFluidCoreCount() {
        return getClientStructureCoreCount(ModBlocks.MEKANICAL_FACTORY_FLUID_CORE.getBlock(),
                fluidCoreCount, MAX_FLUID_CORES);
    }

    public int getCatalystCoreCount() {
        return getClientStructureCoreCount(ModBlocks.MEKANICAL_FACTORY_CATALYST_CORE.getBlock(),
    ...
    private int getClientStructureCoreCount(Block core, int fallback, int maximum) {
        if (!isRemote() || getWorld() == null || getBounds() == null) {
            return fallback;
        }
        ...
        for (int x = min.getX() + 1; x < max.getX(); x++) {
            for (int y = min.getY() + 1; y < max.getY(); y++) {
                for (int z = min.getZ() + 1; z < max.getZ(); z++) {
                    if (getWorld().getBlockState(cursor.set(x, y, z)).is(core)
                            && ++count >= maximum) {
                        return maximum;
                    }
                }
    ...
    private final class FactoryEnergyContainer extends BasicEnergyContainer {
        private FactoryEnergyContainer() {
            super(FloatingLong.MAX_VALUE, BasicEnergyContainer.notExternal,
        ...
        public FloatingLong getEnergy() {
            return MekanicalFactoryMultiblockData.this.isRemote()
                    ? FloatingLong.create(clientEnergy)
                    : super.getEnergy();
        }

        @Override
        public FloatingLong getMaxEnergy() {
            return FloatingLong.create(getEnergyCapacity());
        }
    }
