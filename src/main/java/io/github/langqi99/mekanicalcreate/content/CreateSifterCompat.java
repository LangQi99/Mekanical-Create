...
final class CreateSifterCompat {
    ...
    private static List<SiftingRecipeData> discoverRecipes(Level level) {
        ...
        for (RecipeType<?> type : types) {
            for (RecipeHolder<?> holder
                ...
                read(holder).ifPresent(result::add);
            }
        }
    ...
    private static java.util.Optional<SiftingRecipeData> read(RecipeHolder<?> holder) {
        Recipe<?> recipe = holder.value();
        if (!recipe.getClass().getName().endsWith(".SiftingRecipe")) {
            return java.util.Optional.empty();
        }
        try {
            Ingredient input = invoke(recipe, "getInput", Ingredient.class);
            ItemStack mesh = invoke(recipe, "getMesh", ItemStack.class);
            Boolean waterlogged = invoke(recipe, "isWaterlogged", Boolean.class);
            Boolean advanced = invoke(recipe, "requiresAdvancedSifter", Boolean.class);
            Integer processingTime = invoke(recipe, "getProcessingTime", Integer.class);
            ...
            return java.util.Optional.of(new SiftingRecipeData(holder.id(), input,
