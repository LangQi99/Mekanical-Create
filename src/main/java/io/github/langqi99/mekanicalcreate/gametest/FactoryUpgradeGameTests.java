...
public final class FactoryUpgradeGameTests {
    ...
    public static void tierInstallersPreserveEveryMachineInventory(GameTestHelper helper) {
        ...
        helper.runAfterDelay(2, () -> {
            SimulationChamberBlockEntity original = helper.getBlockEntity(pos);
            ...
            slots.get(1).setStack(new ItemStack(Items.WATER_BUCKET, 1));
            ...
            original.getEnergyContainer().setEnergy(12_345);
            ...
            List<ItemStack> installers = List.of(
                    MekanismItems.BASIC_TIER_INSTALLER.asStack(),
                    MekanismItems.ADVANCED_TIER_INSTALLER.asStack(),
                    MekanismItems.ELITE_TIER_INSTALLER.asStack(),
                    MekanismItems.ULTIMATE_TIER_INSTALLER.asStack());
            List<BlockRegistryObject<?, ?>> expectedBlocks = List.of(
                    ModBlocks.BASIC_MEKANICAL_FACTORY,
            ...
            for (int tier = 0; tier < installers.size(); tier++) {
                ...
                InteractionResult result = installer.useOn(new UseOnContext(helper.getLevel(), player,
                        InteractionHand.MAIN_HAND, installer, hit));
                helper.assertTrue(result.consumesAction(), "factory installer did not apply");
                Block expectedBlock = expectedBlocks.get(tier).get();
                helper.assertTrue(helper.getBlockState(pos).is(expectedBlock),
                        "factory installer created the wrong factory tier");

                SimulationChamberBlockEntity upgraded = helper.getBlockEntity(pos);
                ...
                helper.assertValueEqual(upgraded.getEnergyContainer().getEnergy(), 12_345L,
                        "factory installer lost stored energy");
                helper.assertValueEqual(upgraded.getComponent().getUpgrades(Upgrade.SPEED), 3,
                        "factory installer lost installed upgrades");
                helper.assertValueEqual(upgraded.getComponent().getUpgrades(Upgrade.ENERGY), 2,
    ...
    public static void upgradeSnapshotDoesNotDependOnRemovedMachineSlots(GameTestHelper helper) {
        ...
        helper.runAfterDelay(2, () -> {
            ...
            original.getEnergyContainer().setEnergy(54_321);

            IUpgradeData snapshot = original.getUpgradeData(helper.getLevel().registryAccess());
            for (mekanism.api.inventory.IInventorySlot slot : oldSlots) {
                slot.setStack(ItemStack.EMPTY);
            }
            original.getEnergyContainer().setEnergy(0);

            helper.setBlock(pos, ModBlocks.BASIC_MEKANICAL_FACTORY.get());
            SimulationChamberBlockEntity upgraded = helper.getBlockEntity(pos);
            upgraded.parseUpgradeData(helper.getLevel().registryAccess(), snapshot);
            ...
            helper.assertValueEqual(upgraded.getEnergyContainer().getEnergy(), 54_321L,
