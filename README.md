...
- **动力冲压机**：压片与冲压配方。
- **动力锯**：切割配方。
- **鼓风机**：搭配水、熔岩、灵魂篝火或普通篝火标记执行内建鼓风加工；按槽位顺序只处理本轮四格输出能够完整容纳的数量，剩余材料留到下一轮。多产物和概率副产物会作为单件原子结果预留，不会因整批过大而卡住或吞料；输出受阻时不耗电也不推进进度。耗时与 Create 原生鼓风加工一致，并遵循每 16 件一档的计时规则；无配方物品不会被熔岩条件吞掉。
- **石磨与粉碎轮**：研磨、粉碎及概率副产物。
...
每条生产线独立推进进度并消耗能量，同时共享工厂的无序输入池与输出缓冲；工厂等级本身只增加并行数，单条生产线的速度与耗电仍由 Mekanism 升级组件控制。
...
- 速度核心：放在内部，每个增加 100% 基础处理速度，不设人工上限；配方达到每刻一次后，多余速度会转换为平均并行处理次数。
- 能量核心：放在内部，每个增加 100 kFE 储能，不设人工上限。
